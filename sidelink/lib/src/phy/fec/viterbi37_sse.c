...
void set_viterbi37_polynomial_sse(int polys[3]) {
  ...
  for(state=0;state < 32;state++){
    Branchtab37_sse2_2[0].c[state] = (polys[0] < 0) ^ parity((2*state) & polys[0]) ? 255:0;
    Branchtab37_sse2_2[1].c[state] = (polys[1] < 0) ^ parity((2*state) & polys[1]) ? 255:0;
    Branchtab37_sse2_2[2].c[state] = (polys[2] < 0) ^ parity((2*state) & polys[2]) ? 255:0;
  }
}
  ...
  while(nbits--) {
   ...
   // printf("nbits=%d, syms=%d,%d,%d\n", nbits, syms[0], syms[1], syms[2]);fflush(stdout);
    ...
    for(i=0;i<2;i++){
      ...
      /* Form branch metrics */
      m0 = _mm_avg_epu8(_mm_xor_si128(Branchtab37_sse2_2[0].v[i],sym0v),_mm_xor_si128(Branchtab37_sse2_2[1].v[i],sym1v));
      metric = _mm_avg_epu8(_mm_xor_si128(Branchtab37_sse2_2[2].v[i],sym2v),m0);
