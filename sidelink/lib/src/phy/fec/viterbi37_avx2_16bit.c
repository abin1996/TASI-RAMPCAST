...
void set_viterbi37_polynomial_avx2_16bit(int polys[3]) {
  ...
  for(state=0;state < 32;state++){
    Branchtab37_sse2_16bit[0].c[state] = (polys[0] < 0) ^ parity((2*state) & polys[0]) ? 65535:0;
    Branchtab37_sse2_16bit[1].c[state] = (polys[1] < 0) ^ parity((2*state) & polys[1]) ? 65535:0;
    Branchtab37_sse2_16bit[2].c[state] = (polys[2] < 0) ^ parity((2*state) & polys[2]) ? 65535:0;
  }
}
  ...
  while(nbits--) {
   ...
   // printf("nbits=%d, syms=%d,%d,%d\n", nbits, syms[0], syms[1], syms[2]);fflush(stdout);
    ...
    for(i=0;i<2;i++){
      ...
      /* Form branch metrics */
      m0 = _mm256_avg_epu16(_mm256_xor_si256(Branchtab37_sse2_16bit[0].v[i],sym0v),_mm256_xor_si256(Branchtab37_sse2_16bit[1].v[i],sym1v));
      metric = _mm256_avg_epu16(_mm256_xor_si256(Branchtab37_sse2_16bit[2].v[i],sym2v),m0);
