  ...
  while ((nf < nof_frames || nof_frames == -1) && !go_exit) {
    for (sf_idx = 0; sf_idx < SRSLTE_NOF_SF_X_FRAME && (nf < nof_frames || nof_frames == -1); sf_idx++) {
      ...
      //if ( (((sfn*SRSLTE_NOF_SF_X_FRAME + sf_idx) % repo.syncPeriod) != repo.syncOffsetIndicator_r12)) {
        ...
        printf("tti: %d ssf: %d\n", sfn*SRSLTE_NOF_SF_X_FRAME + sf_idx, ssf);
      ...
      if (send_data) {
        if(sf_idx != 1 || mbsfn_area_id < 0) { // PDCCH + PDSCH
          ...
          srslte_dci_msg_pack_pdsch(&dci_dl, dci_format, &dci_msg, cell.nof_prb, cell.nof_ports, false);
          ...
          srslte_ra_dl_dci_to_grant(&dci_dl, cell.nof_prb, UE_CRNTI, &grant);        
          ...
          /* Encode PDCCH */
          INFO("Putting DCI to location: n=%d, L=%d\n", locations[sf_idx][0].ncce, locations[sf_idx][0].L);
          srslte_dci_msg_pack_pdsch(&dci_dl, SRSLTE_DCI_FORMAT1, &dci_msg, cell.nof_prb, cell.nof_ports, false);
