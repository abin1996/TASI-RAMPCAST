...
import re
import messaging.messaging.helpers.routecalc
from messaging.messaging.helpers.routecalc import Graph, OpenStreetGraph
...
class MsgPriorityQueue(queue.PriorityQueue):
    def __init__(self):
        ...
        self.counter = 0
        self.message_cache = {}  # Dictionary to store messages by a unique key

    def put(self, item, priority):
        # Create a unique key for each message
        #message_key = f"{item['device_nbr']}_{item['direction']}_{item['interstate']}"
        message_key = item['device_nbr']

        # Check if a message with the same key exists in the cache
        if message_key in self.message_cache:
            # Get the existing message
            existing_priority, counter, item = self.message_cache[message_key]
            publish_time = item['publish_time']
            current_time = time.time()
            time_difference_minutes = (current_time - publish_time) / 60
            ...
            if time_difference_minutes > 5:
                ...
                try:
                    self.pq.queue.remove((existing_priority, counter, item))
                except ValueError:
                    pass  # Ignore if the item is not found in the queue

                self.pq.put((priority, self.counter, item))

                # Update the cache with the new message
                self.message_cache[message_key] = (priority, self.counter, item)
        else:
    ...
    def get(self):
        current_priority, counter, item = self.pq.get()
        #message_key = f"{item['device_nbr']}_{item['direction']}_{item['interstate']}"
        message_key = item['device_nbr']
        current_repeat = item['repeat']

        print(f"Repeat:{current_repeat}, priority: {current_priority}, item: {item}, counter :{counter} \n")

        # Fading priority implementation
        if current_repeat > 0 :
            item['repeat'] = current_repeat -1
            current_priority= current_priority + 2 # lower the priority for next round
            self.counter += 1
            self.pq.put((current_priority, self.counter, item))


        print("No of items in queue: ", str(self.pq.qsize()))

        existing_priority, counter, item_cache = self.message_cache[message_key]
        publish_time = item_cache['publish_time']
        current_time = time.time()
        time_difference_minutes = (current_time - publish_time) / 60
        ...
        if time_difference_minutes > 5:
            ...
            #Remove the message from the cache when it's dequeued
            if message_key in self.message_cache:
                del self.message_cache[message_key]

        print("Message cache:", self.message_cache)
        return item
...
class UserInterface(threading.Thread):
    ...
    def display_image(self):
        ...
        if tts_message:
            priority_tts = tts_message.get('priority', 0)
        ...
        if dms_message:
            priority_dms = dms_message.get('priority', 0)
            # print(dms_message)
        ...
        if (priority_tts < 2) or (priority_dms < 2):
            time.sleep(4)
        else:
            time.sleep(2)
    ...
    def read_from_queue_and_display(self):
        ...
        if not self.ui_queue.empty():
            ...
            if ui_message['message_type'] == 'tts':
                ...
                self.ui_direction_cache[ui_message.get('direction')][ui_message.get('interstate')]['tts'] = ui_message
                ...
                self.ui_direction_cache[ui_message.get('direction')][ui_message.get('interstate')]['dms'] = ui_message
    ...
    def load_image(self, device_nbr, travel_times, composite_image, additional_mile_values, interstate_value, interstate_direction):
...
class MqttHandler(threading.Thread):
    def __init__(self, thread_id, vehicle_name, gps_thread, ui_message_queue, is_local, save_message=False):
        ...
        self.graph = OpenStreetGraph(filepath='/home/tasi/TASI-RAMPCAST/messaging/messaging/helpers/indiana_route.graphml')
    ...
    def handle_travel_time_message(self, msg_json):
        ...
        ui_message = {
            ...
            "publish_time": msg_json['publish_time'],
            "repeat": msg_json['repeat']
        }
        priority = msg_json.get('priority', 0)
    ...
    def handle_text_message(self, msg_json):
        ...
        # Define a regular expression pattern to match the numerical value before "MILES AHEAD"
        pattern = r'(\d+(\.\d+)?|\.\d+)\s*MILES\s*AHEAD'
        match = re.search(pattern, input_text)

        if match:
            # Extract the matched numerical value
            #old_value = match.group(1)
            # Replace the old_value with the new value (e.g., replace 79.4 with 100)
           # print("matched value:", old_value)

            event_coordinates = (msg_json['geometry']['coordinates'][1], msg_json['geometry']['coordinates'][0])
            rsu_coordinates = (msg_json['rsu_coordinates'][0], msg_json['rsu_coordinates'][1])

            distance = self.graph.shortest_road_distance(event_coordinates,  rsu_coordinates )

            modified_text = re.sub(pattern, f"{ round(float(distance), 1)} MILES AHEAD", input_text)
        else:
            # If the pattern is not found, keep the original input_text
    ...
    def check_message_in_heading_of_vehicle(self, msg_json):
        ...
        if current_loction.lat and current_loction.lon and current_loction.heading:
            ...
            a = math.sin(delta_lat / 2)**2 + math.cos(vehicle_latitude_rad) * math.cos(coord_latitude_rad) * math.sin(delta_lon / 2)**2
            ...
            x = math.cos(vehicle_latitude_rad) * math.sin(coord_latitude_rad) - math.sin(vehicle_latitude_rad) * math.cos(coord_latitude_rad) * math.cos(delta_lon)
...
class GPSHandler(threading.Thread):
    ...
    def set_bearing(self):
        ...
        if self.cached_gps_value.lat and self.cached_gps_value.lon:
            brng = Geodesic.WGS84.Inverse(self.cached_gps_value.lat, self.cached_gps_value.lon, self.active_gps_value.lat, self.active_gps_value.lon)['azi1']
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--save-message", required=False, default=False, action="store_true", help="Save messages")
