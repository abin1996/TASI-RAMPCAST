...
class UserInterface(threading.Thread):
    def __init__(self, thread_id ,ui_priority_queue, window, label):
        threading.Thread.__init__(self)
        self.thread_id = thread_id
        self.speedometer_image = Image.open(SPEEDOMETER_IMAGE_PATH)
        self.window = window
        self.label = label
        self.ui_priority_queue = ui_priority_queue
    ...
    def display_image(self):
        ...
        if tts_message:
            self.load_image(tts_message.get('device_nbr'), tts_message.get('travel_times'), self.base_image, tts_message.get('mile_values'), tts_message.get('interstate'), tts_message.get('direction'))
    ...
    def read_from_queue_and_display(self):
        if self.ui_priority_queue:
            # Get the message with the highest priority
            top_priority_message = heapq.heappop(self.ui_priority_queue)

            # Print the priority value and message content to the terminal
            self.load_base_image()
            priority = top_priority_message.get('priority', 0)
            message_content = top_priority_message.get('message_type', '') + ': ' + top_priority_message.get('input_text', '')
            print(f"Message Priority: {priority}, Content: {message_content}")

            if top_priority_message['message_type'] == 'tts':
                self.ui_cache['tts'] = top_priority_message
                self.ui_direction_cache[top_priority_message.get('direction')][top_priority_message.get('interstate')]['tts'] = top_priority_message
                prev_dms = self.ui_direction_cache[top_priority_message.get('direction')][top_priority_message.get('interstate')]['dms']
                if prev_dms:
                    self.ui_cache['dms'] = prev_dms
            else:
                self.ui_cache['dms'] = top_priority_message
                self.ui_direction_cache[top_priority_message.get('direction')][top_priority_message.get('interstate')]['dms'] = top_priority_message
                prev_tts = self.ui_direction_cache[top_priority_message.get('direction')][top_priority_message.get('interstate')]['tts']
                if prev_tts:
...
class MqttHandler(threading.Thread):
    def __init__(self, thread_id, vehicle_name, gps_thread, ui_priority_queue, is_local, save_message=False):
        threading.Thread.__init__(self)
        self.thread_id = thread_id
    ...
    def handle_travel_time_message(self, msg_json):
        ...
        priority = msg_json.get('priority', 0)
        #self.ui_message_queue.put(ui_message)
        heapq.heappush(self.ui_priority_queue, (priority, ui_message))
...
if __name__ == "__main__":
    ...
    try:
        ...
        gps_thread.start()
        ui = UserInterface("ui_thread", ui_priority_queue, window, label)
        ui.daemon=True
        mqtt_handler = MqttHandler("mqtt_thread",args.vehicle_name, gps_thread,ui_priority_queue, args.is_local, args.save_message)
