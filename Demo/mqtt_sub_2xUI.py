...
class MsgPriorityQueue(queue.PriorityQueue):
    ...
    def get(self):
        ...
        print(f"Repeat:{current_repeat},item: {item}, counter :{counter} \n")
...
class UserInterface_Tab(threading.Thread):
    def __init__(self, thread_id, ui_queue, window, label):
        threading.Thread.__init__(self)
        self.thread_id = thread_id
        self.speedometer_image = Image.open(SPEEDOMETER_IMAGE_PATH)
        ...
        self.ui_queue = ui_queue
        self.ui_cache = {'tts': None, 'dms': None}
        self.bk_window_x = 360
        self.bk_window_y = 500
        self.ui_direction_cache = {
            'NB': {
                65: {
                    'tts': None,
    ...
    def load_base_image(self):
        ...
        self.base_image = Image.new('RGBA', (self.bk_window_x, self.bk_window_y), (0, 0, 0, 255))
    ...
    def load_image(self, device_nbr, travel_times, composite_image, additional_mile_values, interstate_value,
        ...
        text_width1, text_height1 = draw.textsize(travel_time1, font=font)
        text_x1 = (self.bk_window_x // 2) - 70  # =530
        text_y1 = ((composite_image.height - sign_image.height) // 2) + 50

        text_width2, text_height2 = draw.textsize(travel_time2, font=font)
        text_x2 = (self.bk_window_x // 2) + 20  # =620
        text_y2 = ((composite_image.height - sign_image.height) // 2) + 50

        mile_width1, mile_height1 = draw.textsize(mile_value1, font=mile_font)
        mile_x1 = (self.bk_window_x // 2) - 82  # =518
        mile_y1 = text_y1 - mile_height1 - 28

        mile_width2, mile_height2 = draw.textsize(mile_value2, font=mile_font)
        mile_x2 = (self.bk_window_x // 2) + 8  # =608
        mile_y2 = text_y2 - mile_height2 - 28
        ...
        interstate_text = f"I-{interstate_value} {interstate_direction}"
        text_width, text_height = draw.textsize(interstate_text, font=font)
        text_x = (self.bk_window_x - text_width) // 2
        text_y = 20
        draw.text((text_x, text_y), interstate_text, fill='white', font=font)
...
class MqttHandler(threading.Thread):
    def __init__(self, thread_id, vehicle_name, gps_thread, ui_message_queue1, ui_message_queue2, is_local,
                 save_message=False):
        threading.Thread.__init__(self)
        self.thread_id = thread_id
        self.client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1,vehicle_name, clean_session=False, reconnect_on_failure=True)
        self.client.username_pw_set('mqtt-test1', 'tasi')
        self.is_local = is_local
        self.save_message = save_message
        self.client.on_connect = self.on_connect
        self.client.on_message = self.on_message
        self.ui_message_queue1 = ui_message_queue1
        self.ui_message_queue2 = ui_message_queue2
    ...
    def handle_travel_time_message(self, msg_json):
        ...
        priority = msg_json.get('priority', 0)
        self.ui_message_queue1.put(ui_message, int(priority))
        self.ui_message_queue2.put(ui_message, int(priority))
...
if __name__ == "__main__":
    ...
    # Create separate message queues for each UI thread
    ui_message_queue1 = MsgPriorityQueue()
    ui_message_queue2 = MsgPriorityQueue()

    # Create two GUI windows with their own labels
    window1 = tk.Tk()  # First GUI window
    window1.title(UI_WINDOW_TITLE)
    label1 = tk.Label(window1)
    label1.pack()

    window2 = tk.Toplevel()  # Second GUI window
    window2.title(UI_WINDOW_TITLE)
    ...
    try:
        ...
        ui1 = UserInterface("ui_thread1", ui_message_queue1, window1, label1)
        ui1.daemon = True

        ui2 = UserInterface_Tab("ui_thread2", ui_message_queue2, window2, label2)
        ui2.daemon = True

        mqtt_handler1 = MqttHandler("mqtt_thread1", args.vehicle_name, gps_thread, ui_message_queue1, ui_message_queue2,
                                    args.is_local, args.save_message)
        mqtt_handler1.daemon = True
        mqtt_handler1.start()

        ui1.start()
        ui2.start()

        # mqtt_handler2 = MqttHandler("mqtt_thread2", args.vehicle_name, gps_thread, ui_message_queue2, args.is_local, args.save_message)
        # mqtt_handler2.daemon = True
        # mqtt_handler2.start()
